...
namespace canyon::graphics::vulkan {
    ...
    namespace {
        ...
        LayoutTransitionInfo GetTransitionInfo(VkImageLayout oldLayout, VkImageLayout newLayout) { // NOLINT(readability-function-cognitive-complexity)
            ...
            if (oldLayout == VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL && newLayout == VK_IMAGE_LAYOUT_PRESENT_SRC_KHR) {
                return {
                    VK_ACCESS_TRANSFER_READ_BIT,
                    VK_ACCESS_MEMORY_READ_BIT,
                    VK_PIPELINE_STAGE_TRANSFER_BIT,
    ...
    void CommandBuffer::TransitionImageLayout(Texture& image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout) {
        ...
        // this nonsense needs rethinking. maybe i should figure out how this works?
        auto const info = GetTransitionInfo(oldLayout, newLayout);
        ...
        vkCmdPipelineBarrier(m_vkCommandBuffer, info.srcStage, info.dstStage, 0, 0, nullptr, 0, nullptr, 1, &barrier);
    }
    ...
    void CommandBuffer::CopyImageToImage(Texture& srcImage, Texture& dstImage) {
        ...
        region.extent.width = dstImage.GetVkExtent().width;
        region.extent.height = dstImage.GetVkExtent().height;
